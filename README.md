...
Deck | match | This method is supposed to check if the card that the player has decided to play matches any of the following criteria  Is a wild card, Matches the colour of the top card of the deck, Matches the number of the top cards of the deck, Return False values if none of the above were satisfied. | The test should return a dictionary containing {"match":True,"wild":False} when a card matches the top card but isn't a wild card | The method returned {"match":True,"wild":False} when a card matched the top card of the deck and wasn't a wild card | NA | 
