...
Deck | match | Test if the match method returns the correct dictionary when a card that isn't a wild card is played and it matches the top card of the deck | The test should return a dictionary containing {"match":True,"wild":False} when a card matches the top card but isn't a wild card | The method returned {"match":True,"wild":False} when a card matched the top card of the deck and wasn't a wild card | NA | 
